...
### トマト収穫判定（色の実測 + 固有カウント + 収穫日数）

`tomato-harvest` プロファイル。温室のトマトを検出・追跡し、実ごとに熟度と収穫までの目安日数を出します。

* 映像: Voice of America の取材映像（パブリックドメイン、`public/demo/tomato-harvest/CREDIT.txt`）
* 検出・追跡: Grounding DINO（プロンプト "tomato"、6 フレームごと）+ IoU 追跡の事前追跡結果（`npm run track:tomato`）
* **熟度は実測**: 枠の中央の画素を色相で採点（赤 1.0 / 橙 0.75 / 黄 0.45 / 黄緑 0.2 / 緑 0）し、
  移動平均した値を熟度 0〜1 にする（`src/services/ripenessMeter.ts`）。学習済みの熟度分類モデルは使っていません。
* **収穫までの日数は表引き**（緑 14 日 / 半熟 6 日 / 完熟 0 日を線形補間）。品種・気候で変わる目安であり、生育速度の実測ではありません。
* **固有カウント**: 追跡 ID を 1 個 1 回だけ数え、フレームごとの検出数は数えません（映像左上に「固有ID数（重複計上なし）」）。
  同じ実が画面外に出て戻ると別 ID になる（長期の再同定はしない）のは参照実装と同じ制約です。
* 判定は計測プロファイルと同じ規則: 熟度 ≥ 0.9 A 収穫適期 / 0.8〜0.9 B 収穫可 / 0.68〜0.8 C 色づき中（再確認）/
  0.52〜0.68 D 色づき始め（人の確認）/ < 0.52 E 未熟（収穫見送り）。トリガーはゾーン滞留 0.8 秒。
* 未熟の割合は異常ではないので、このプロファイルではライン判断と異常警報を行いません（`lineMonitoring: false`）。

### ジャガイモ計数・選別（1 フレーム注釈から学習した専用モデル + 計数ライン + サイズ選別）

`potato-count` プロファイル。USDA の種芋選別映像（パブリックドメイン）で、ベルト上のジャガイモを計数ラインで数え、相対サイズで選別します。

* **ゼロショット検出器は効かなかった**: YOLO-World / Grounding DINO は密集した土色のジャガイモを 0〜数個しか検出できませんでした。
* **1〜2 フレームから学習**（参照記事の再現）: `scripts/annotate_sam.py` が SAM 2（tiny）に土色の画素から選んだ点プロンプトを渡して
  1 個ずつマスクにし（2 フレームで 137 枠）、`scripts/train_from_frames.py` がその 2 枚だけで YOLO11n を学習します（CPU で数分）。
  学習したモデル + BoT-SORT で全フレームを追跡した結果が `detections.json` です（`npm run track:potato`）。
* **計数**: 追跡 ID が計数ライン（y = 0.55）を横切った回数を数えます。フレームごとの検出数は数えません。
* **サイズ選別は実測**: 追跡枠の √面積 を全枠の中央値で割った相対サイズ（基準比 1.00 ± 0.30 が規格内）。重量や実寸ではありません。
  A 基準サイズ / B 規格内 / C 境界（再計測）/ D 逸脱（人の確認）/ E 規格外（小玉・大玉）。
...
* **B 率による先行注視**: ライン判断に「許容（B）率」を渡し、不良率が基準内でも B が 35% を超えたら「注視」に上げます
  （理由 `MARGINAL_RATE_ELEVATED`）。不良が出る前の兆候を拾うためのものです。
* **グレード境界の調整**: 左列「グレード境界」パネルで B / C / D / E の境界（異常度）をプロファイルごとに動かせます。
  順序と最小間隔は自動で保たれ、変更は必ずイベントログに「誰が・何を・いくつからいくつへ」として残り、
  ブラウザに保存されます（`localStorage`）。異常度への写像（属性信頼度 → 異常度など）は固定で、動くのは境界だけです。
* **時系列プロファイルの証拠窓**: 転倒・横断歩道では状態の継続時間に合わせた直近の窓（0.25〜1.5 秒）だけを安定度の材料にします。
* **人の判定のフィードバック**: 確認待ちで人が下した合格 / 不良をグレード別に集計し、
  「D の 60% 以上を人が合格にしている → D 境界を上げる余地」のような見直しの根拠を確認待ちパネルに表示します。
