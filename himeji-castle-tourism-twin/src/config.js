...
const CONFIG = window.TWIN_CONFIG = {
  ...
  las:        { originXY:null },
  pcView:     { budget:900000 },   // ◆ 点群ビューのファサード点の上限   // LAS がメートル座標のとき、シーン中心（姫路城）に当たる [E, N]。null＝重心を城に合わせる   // Human Flow API（infra/api）。base 空＝同一オリジン（API 配信時）または http://localhost:8000
};
