...
| 操作 | 動き |
| --- | --- |
| 左ドラッグ | 地図を掴んで引っ張る（移動）。右下「移動中」ボタンで左右の役割を入替 |
| 右／中ドラッグ、Shift／Ctrl＋ドラッグ | 360度回転・上下で真上〜横まで傾ける（現在の画面中心を軸に回る） |
| ホイール（縦） | カーソル位置に向かってズーム |
| ホイール（横・トラックパッド） | 横スクロール |
| 右下ツール | 戻る（姫路城中心）／2D（真上）／横／接近（大手前通り）／＋－／集中（UIを隠す）／設定（表示のデザイン） |
| 右上「表示のデザイン」 | 街の表現（立体／複合／点群）、補完点群の密度、人流の表現（粒子／軌跡／動線）、軌跡の強さ、再生速度 |
