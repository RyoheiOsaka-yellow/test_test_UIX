  ...
  return (
    <Panel title="システム" bodyClassName="px-2.5 py-1.5">
      ...
      <Field
        ...
        value={profile.trigger.kind === 'state' ? (profile.analyzer === 'crosswalk' ? '場面解析（ゾーン + 車両接近）' : '時系列判定（5特徴量）') : profile.measurement ? (profile.measurement.method === 'ripeness' ? '実測（色相の画素解析）' : '実測（画素の HSV 解析）') : profile.severityFromArea ? '検出枠の面積から算出' : '疑似注入'}
        mono={false}
      />
      <Field
        label="判定トリガー"
