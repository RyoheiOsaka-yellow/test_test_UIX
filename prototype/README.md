...
| おまつり WINKY | 野球 WINKY | 走る |
|---|---|---|
| ![おまつり](docs/screenshot_idle.png) | ![野球](docs/screenshot_bat.png) | ![走る](docs/screenshot_run.png) |

キャラクターは提供イラスト(白い鳥の着ぐるみ+水色ボディの WINKY)を
トゥーン調で立体化したものです。品質面では以下を実装しています:

- **均一太さの黒アウトライン**: 頂点を法線方向に一定距離ふくらませる方式。
  パーツの大小によらずイラストのような均一な輪郭線になる
- **3 階調セルシェーディング**: `MeshToonMaterial` + グラデーションマップ
- **顔のディテール**: 三日月形の目のハイライト、目じりのまつ毛、
  Canvas 描画のふんわりグラデーションチーク
- **小物のテクスチャ**: うちわは Canvas に骨・縁取り・「祭」を描画、
  おなかの WINK はアーチ状の手書き風配置
- **細部の造形**: 鈴のスリット、手のひら・足裏の肉球、着ぐるみの尻尾の羽根
- **二次アニメーション**: 体の上下動に遅れて反応するアンテナのばね揺れ、
  翼のはためき・尻尾の揺れ

**アニメーションの滑らかさ**(作り込み)

- 減衰バネ物理(ばね定数+減衰係数)による揺れもの: アンテナ・鳥の頭・
  尻尾・鈴が体の動きに遅れて追従し、余韻を残して揺れる
- 走り出しは現在の向きとコース角度を同期、停止時は ±π に正規化した
  最短方向へ向き直る(巻き戻り回転なし)
- 弾みは sin² カーブ(|sin| の折り返しのカクつきを排除)、走行中は
  カーブへ内傾(バンク)
- ループ状態のブレンドウェイトを smoothstep 化し、遷移の入り・抜きを軟化
- スピンに予備動作(逆ひねり)を追加
- ランダム間隔のまばたき(左目)と待機中の翼のはためき
- キャラ切り替え時は持ち物が easeOutBack でポンッと現れる
...
現在の 3D モデルは、後から提供された PNG イラスト(白い鳥の着ぐるみ・翼・ウインク顔・
黄色い前髪・鈴・おなかの WINK 文字・肉球)を基に、トゥーン調で立体化したものです。
ベクターデータ(SVG)があれば、輪郭をそのまま押し出したより忠実な形状にもできます。
