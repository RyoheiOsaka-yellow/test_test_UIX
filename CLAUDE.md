...
P5（For-Each）は P4 の内側グラフ機構を本体として再利用する（SPEC.md §19）。
実データの棟別処理は `sitectx` の prim 属性 `bid` を piece キーにする。
1,088棟の反復合計 142ms。ピース間で point は複製される（§19.3）。

P6.5（ビューア強化）でピッキング・差分更新・静的シャドウが入った（SPEC.md §20）。
シーン層の契約は `inspect` を足して3メソッドになったが、GeoSet は依然として
`geo` kind の中に閉じている。LOD は意図して見送り（§20.4 に判断理由）。

人流は需要ベースの流入モデルになった（`roadagents` の `mode:'demand'`、SPEC.md §21）。
OSM 歩行網は素のままでは断片化している（両駅とも会場と非連結だった）。
