...
- **スマホでも動きます**（幅での除外はしません）。ただし手加減します（下記）
- **出さない条件**: 省データ設定／`prefers-reduced-motion`／WebGL2 非対応
  → この場合は従来どおり静止画が出ます。静止画は `--bg` のままなので、デモを置いても差し替え不要です

### スマホでの手加減

`demos.json` の `mobile` で決めます。

```json
"mobile": { "dpr": 1.25, "skip": ["hicity"] }
```

- `dpr` … 描画解像度の上限。多くのデモは端末の画素密度をそのまま描画に使うため、
  3倍の端末では9倍の画素を描くことになります。1.25 に抑えると描画量が約6分の1です
- `skip` … スマホでは動かさないデモ。データが大きいものを外します（ポスターのまま）

このほかスマホでは自動的に、**視差を切る**／**画面外に出たデモを破棄する**
（PCでは `keep` で保持）という2点が効きます。
