...
- **スマホでも動きます**（幅での除外はしません）。視差だけはスマホでは切ります
- **出さない条件**: 省データ設定／`prefers-reduced-motion`／WebGL2 非対応
...
- 待ち時間は `add-boot.js` の `MIN`（最短表示 0.8秒）と `MAX`（最長 7秒）で調整します
- スマホでも同じく、3D都市が出てからカーテンが上がります。
  デモが動かない環境（省データ設定等）では、ページの読み込み完了で上がります
