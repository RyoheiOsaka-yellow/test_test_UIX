...
const LS='brex_ext_params_v15';
const P=Object.assign({roof:'flat', rise:7, axis:'x', roofColor:'white', band:false, pitch:1.6, bandH:3.0, bandY:2.4,
...
function panelHTML(){
  return '<div class="sec" id="fac-sec"><div class="sec-t"><b>🏛 外観調整</b> — 実写トレース（写真と照合して調整）</div>'
   ...
   +'<div class="sec-t" style="margin-top:8px">カーテンウォール（周回ガラス帯）</div>'
   +'<div class="row-btns"><button class="chip '+(P.band?'active':'')+'" id="fac-band">周回ガラス帯を表示</button></div>'
...
function bind(){
  ...
  const fb=document.getElementById('fac-band'); if(fb) fb.onclick=()=>{ P.band=!P.band; save(); applyBand(); renderPanel(); };
  ...
  document.getElementById('fac-reset').onclick=()=>{
    ...
    Object.assign(P,{roof:'flat',rise:7,axis:'x',roofColor:'white',band:false,pitch:1.6,bandH:3.0,bandY:2.4});
