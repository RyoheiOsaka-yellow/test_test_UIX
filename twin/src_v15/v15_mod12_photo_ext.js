...
const aSign=makeLabel('宇都宮市体育館', 5.5, '#ffffff'); aSign.position.set(AX-AW/2-1.2, AH-1.2, AZ); G.add(aSign);
/* 外階段（レンガ・広場からテラスへ） */
for(let k=0;k<12;k++){
...
const archLb=makeLabel('WELCOME TO BREX HOME GAME', 9, '#ffffff'); archLb.position.set(PLAZA.x-14, 10.5, CZ); G.add(archLb);
/* マスコット（ブレッキー）: 簡略造形 */
(function(){
  ...
  const lb=makeLabel('ブレッキー', 5, '#f5c400'); lb.position.set(PLAZA.x-6, 6.8, CZ-9); G.add(lb);
})();
/* 旗ポール×3 */
