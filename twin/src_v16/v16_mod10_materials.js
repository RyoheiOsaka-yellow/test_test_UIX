...
(function(){
'use strict';
const LS='brex_mat_params_v15';
const M=Object.assign({real:true, tex:true, led:true,
  pal:{floor:'#15171f', roll:'#1c1f2a', w2lo:'#d9662a', w2hi:'#3f8f4a', ns2:'#3f8f4a', f3:'#22252e'}},
...
const PRESETS={
  photo:{name:'写真準拠（仮）', pal:{floor:'#15171f', roll:'#1c1f2a', w2lo:'#d9662a', w2hi:'#3f8f4a', ns2:'#3f8f4a', f3:'#22252e'}},
  nation:{name:'BREX NATION シャツ演出', pal:{floor:'#15171f', roll:'#f5c400', w2lo:'#f5c400', w2hi:'#f5c400', ns2:'#f5c400', f3:'#f5c400'}},
  mono:{name:'無彩色', pal:{floor:'#1c1f2a', roll:'#2b3140', w2lo:'#2b3140', w2hi:'#2b3140', ns2:'#2b3140', f3:'#22252e'}},
};
...
function panelHTML(){
  ...
  return '<div class="sec" id="mat-sec"><div class="sec-t"><b>🎨 内観素材</b> — 座席実色・テクスチャ・LED演出</div>'
   ...
   +'<div class="row-btns" style="margin-top:6px">'+Object.entries(PRESETS).map(([k,v])=>'<button class="chip" data-mp="'+k+'">'+v.name+'</button>').join('')+'</div>'
   +'<div class="hint" style="margin-top:6px">座席色は写真（2Fスタンド: オレンジ／グリーン、上層: チャコール）から読み取った<b>仮配色</b>。色見本で調整・ブラウザに保存。実色は「リアル観客ビュー」時のみ適用され、分析レイヤーの色分けは維持されます。</div></div>';
}
function bind(){
  ...
  sec.querySelectorAll('[data-mp]').forEach(b=> b.onclick=()=>{ M.pal=Object.assign({}, PRESETS[b.dataset.mp].pal); save(); repaintSeats(); renderPanel(); toast('座席配色: '+PRESETS[b.dataset.mp].name); });
}
