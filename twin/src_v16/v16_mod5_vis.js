...
window.__svTurn=function(dx,dy){ if(!SV.active) return; SV.yaw-=dx*0.0030*(SV.fov/52); SV.pitch=Math.max(-1.15,Math.min(1.15,SV.pitch-dy*0.0026*(SV.fov/52))); };
window.__svZoom=function(k){ if(!SV.active) return; SV.fov=Math.max(16,Math.min(75,SV.fov*k)); };
