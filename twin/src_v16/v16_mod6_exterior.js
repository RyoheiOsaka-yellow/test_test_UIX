...
ext.add(band);
window.__extApi={group:ext, roofCap:null, band, bandMat:glassBandMat, roofMat:null, A, mainMinX, mainMinY, outerMinX, cx0, cy0, setNight:null};

/* 大屋根キャップ＋BREXイエロー屋根ライン＋屋上設備 */
const roofMat=new THREE.MeshStandardMaterial({color:0x2e3548, roughness:0.42, metalness:0.55});
window.__extApi.roofMat=roofMat;
const roofCap=extrudePoly(A.main, 1.3, roofMat, 20.0); ext.add(roofCap); window.__extApi.roofCap=roofCap;
...
for(let z=-15; z<=15; z+=6){
  ...
  c.position.set(outerMinX-5.6, 3.4, MAIN_C.z+z); ext.add(c); canCols.push(c);
}
window.__extApi.dress={canopy, canUnder, canCols, sW, barW, sS, barS};
...
setNight(0); window.__extApi.setNight=setNight;
