...
(function meshes() {
  /* 座席（座面 + 背もたれ） */
  /* ローカル +z がコート方向。座面・座面前縁・背もたれ(2段・後傾)・肘掛け・脚 = 12 面 */
  const B = new Builder(), c = [1, 1, 1], cd = [0.72, 0.72, 0.76];
  B.quad([-0.21, 0, -0.17], [0.21, 0, -0.17], [0.21, 0, 0.22], [-0.21, 0, 0.22], c);            // 座面
  B.quad([-0.21, -0.05, 0.22], [0.21, -0.05, 0.22], [0.21, 0, 0.22], [-0.21, 0, 0.22], cd);      // 座面前縁
  B.quad([-0.21, 0.02, -0.17], [0.21, 0.02, -0.17], [0.21, 0.26, -0.20], [-0.21, 0.26, -0.20], c); // 背 下段
  B.quad([-0.21, 0.26, -0.20], [0.21, 0.26, -0.20], [0.21, 0.50, -0.25], [-0.21, 0.50, -0.25], c); // 背 上段
  B.quad([0.21, 0.02, -0.21], [-0.21, 0.02, -0.21], [-0.21, 0.50, -0.29], [0.21, 0.50, -0.29], cd); // 背 裏面
  for (const s of [-1, 1]) {                                                                   // 肘掛け
    B.quad([s * 0.235 - 0.02, 0.24, -0.17], [s * 0.235 + 0.02, 0.24, -0.17], [s * 0.235 + 0.02, 0.24, 0.14], [s * 0.235 - 0.02, 0.24, 0.14], cd);
    B.quad([s * 0.255, 0.0, -0.17], [s * 0.255, 0.0, 0.14], [s * 0.255, 0.24, 0.14], [s * 0.255, 0.24, -0.17], cd);
  }
  B.quad([-0.05, -0.42, -0.12], [0.05, -0.42, -0.12], [0.05, -0.05, -0.12], [-0.05, -0.05, -0.12], cd); // 脚（前）
  B.quad([0.05, -0.42, -0.18], [-0.05, -0.42, -0.18], [-0.05, -0.05, -0.18], [0.05, -0.05, -0.18], cd); // 脚（後）
  const g = B.geom();
  const mat = new THREE.MeshStandardMaterial({ roughness: 0.78, metalness: 0.02, side: THREE.DoubleSide });
