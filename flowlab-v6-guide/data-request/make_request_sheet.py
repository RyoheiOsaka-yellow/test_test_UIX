...
Pre-filled rows: only the anonymous IDs in the position logs with their process and
measured days (⑦), taken from the logs themselves. Everything else is left blank.
"""
...
def sheet(title, heading, purpose, columns, rows=100, example=None, calc=None, lists=None, prefill=None, formats=None):
    ...
    ws["A3"] = "黄色＝記入欄（分かる範囲で・空欄可）　灰色＝記入例　緑＝自動計算　青＝位置ログなど、こちらで記入済みの値（違っていれば上書きしてください）"
...
sheet("①ロット実績", "① ロット（製造）実績",
      ...
      [("日付", 12, None), ("シフト", 9, None), ("工程", 16, None), ("ロットNo", 14, "製造指図No・バッチNoなど"),
       ...
       ("使用設備", 18, "例：調合槽 No.1、充填ライン 1"), ("作業人数", 9, None), ("備考", 30, "やり直し・色替え・待ちなど")],
      rows=300,
...
# ---------------- ③ 設備能力 ----------------
sheet("③設備能力", "③ 設備の能力",
      "設備1台が1回（1バッチ）でどれだけ・何分で処理できるか。工程ごとの能力と、どこが詰まるか（律速工程）を出すのに使います。設備台帳・作業標準書の添付でも構いません。",
      [("建屋", 16, None), ("工程", 16, None), ("設備名", 24, None), ("容量", 9, None), ("容量の単位", 9, "L／kg など"),
       ...
       ("能力（充填機など）", 12, "例：20"), ("能力の単位", 10, "例：缶/分"), ("1日の稼働時間（時間）", 11, None), ("備考", 28, "台数・更新予定など")],
      rows=60,
      example=["（例）第一ペイント工場", "調合", "（例）調合槽 No.1", 2000, "L", 1500, "kg", 180, 1, 40, "（例）1日1〜2回", None, None, 8, None],
      lists={2: PROCESSES})
...
sheet("⑦位置ログID", "⑦ 位置ログのIDと担当の対応",
      ...
      [("位置ログのID", 11, None), ("記録上の工程", 20, "位置ログに記録されていた所属"), ("計測日", 22, "位置ログに記録がある日"),
       ("実際の担当工程", 18, "記録と違えば記入"), ("主な担当設備・作業", 28, "例：調合槽、原料の投入"),
...
lines = [
    ...
    ("色の意味", "黄色＝記入欄　灰色＝記入例　緑＝自動計算　青＝位置ログなど、こちらで記入済みの値（違っていれば上書き）"),
]
r = 5
for k, v in lines:
