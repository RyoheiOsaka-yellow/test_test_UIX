...
(function () {
  ...
  const S = {
    ...
    undoStack: [],
    layers: { caries: true, restoration: true, root: false, bone: false,
              perio: false, xray: false },
  ...
  function surfaceColorFor(fdi, rec, surf) {
    ...
    // 治療工程の再生中は、その面だけ工程色で上書きする（データは変えない）
    const sc = stepColorFor(fdi, surf);
  ...
  function recolorAll() {
    ...
    updateBoneLevels();
    if (S.mediaMesh) {
      S.mediaMesh.visible = !!S.layers.xray;
      placeMedia();
    }
  ...
  function bindUI() {
    ...
    document.getElementById('reset').addEventListener('click', function () {
      ...
      clearMedia(); setLayer('xray', false);   // 患者画像はリセットで破棄する
    ...
    document.getElementById('imgFile').addEventListener('change', function (e) {
      const f = e.target.files && e.target.files[0]; if (!f) return;
      const rd = new FileReader();
      // 患者データなので HTML にもストレージにも保存しない（SPEC §5.20 / §8.3）
      rd.onload = function () { setMediaImage(rd.result); };
      rd.readAsDataURL(f);
    });
  ...
  function stopSeq() {
    ...
    if (S.stepSim) {                 // 工程の上書きを破棄して現在の状態へ戻す
      S.stepSim = null;
      const sn = document.getElementById('simNote');
      if (sn) {
        sn.textContent = '治療後シミュレーション表示中（実際の治療結果とは異なります）';
        sn.classList.toggle('hide', !S.simulated);
      }
      S.needsRecolor = true;
    }
  }
  ...
  /* =======================================================================
     治療工程アニメーション — SPEC §5.19 / 研究資料 §9
     所見データは書き換えず、再生中だけ有効な表示上書きとして持つ
     ======================================================================= */
  const STEP_COLORS = { cavity: 0x6B5B4B, prepared: 0x8A7A68 };
  const STEP_TEXT = [
    'いまの状態です',
    '虫歯になっているところを取り除きます',
    '詰め物が入る形に整えます',
  ...
  // 再生中の上書き（{fdi, surf, stage, material}）。終了時に破棄する
  function stepColorFor(fdi, surf) {
    const st = S.stepSim;
    if (!st || st.fdi !== fdi) return null;
    if (normSurf(surf) !== st.surf) return null;
    if (st.stage === 1) return STEP_COLORS.cavity;
    if (st.stage === 2) return STEP_COLORS.prepared;
    if (st.stage >= 3) return CONFIG.COLORS[st.material] || CONFIG.COLORS.CR;
    return null;   // stage 0 は現在の所見色のまま
  }

  function pickStepTarget() {
    // 選択歯に計画があればそれ。無ければ計画の先頭
    const items = planItems().filter(function (p) {
      return p.procedure !== 'OBSERVE' && p.procedure !== 'EXTRACTION';
    });
    if (S.selected != null) {
      const own = items.filter(function (p) { return p.fdi === S.selected; })[0];
      if (own) return own;
    }
    return items[0] || null;
  }
  ...
  function runStepSeq() {
    const p = pickStepTarget();
    if (!p) {
      at(0, function () { narrate('工程を再生できる治療計画がありません'); });
      at(2600, stopSeq);
      return;
    }
    const surf = normSurf((p.surfaces || 'O')[0] || 'O');
    const mat = (p.procedure === 'INLAY') ? 'IN_METAL' : 'CR';
    const t = [0, 2400, 4600, 6800, 9000];

    at(0, function () {
      setSimulated(false); setStage(0);
      S.stepSim = { fdi: p.fdi, surf: surf, stage: 0, material: mat };
      selectTooth(p.fdi);
      const el = document.getElementById('simNote');
      if (el) {
        el.textContent = '治療の工程（シミュレーション。実際の治療結果とは異なります）';
        el.classList.remove('hide');
      }
      narrate(fdiToPalmer(p.fdi) + 'の歯：' + STEP_TEXT[0]);
      S.needsRecolor = true;
    });
    for (let i = 1; i <= 4; i++) {
      (function (k) {
  ...
  /* =======================================================================
     X線・口腔内写真の空間配置 — SPEC §5.20
     画像はローカル読み込みのみ。HTML にもストレージにも保存しない
     ======================================================================= */
  function setMediaImage(dataUrl) {
    const img = new Image();
    img.onload = function () {
      if (S.mediaMesh) {
        S.scene.remove(S.mediaMesh);
        if (S.mediaMesh.material.map) S.mediaMesh.material.map.dispose();
        S.mediaMesh.material.dispose();
        S.mediaMesh.geometry.dispose();
        S.mediaMesh = null;
      }
      const tex = new THREE.Texture(img);
      tex.needsUpdate = true;
      tex.encoding = THREE.sRGBEncoding;
      const long = 34, r = img.height / Math.max(1, img.width);
      const w = r <= 1 ? long : long / r;
      const h = r <= 1 ? long * r : long;
      const mesh = new THREE.Mesh(
        new THREE.PlaneGeometry(w, h),
        new THREE.MeshBasicMaterial({ map: tex, side: THREE.DoubleSide,
                                      transparent: false })
      );
      mesh.name = 'media_plate';
      mesh.visible = !!S.layers.xray;
      S.scene.add(mesh);
      S.mediaMesh = mesh;
      setLayer('xray', true);
      placeMedia();
  ...
  function placeMedia() {
    const m = S.mediaMesh;
    if (!m) return;
    const t = (S.selected != null) ? S.teeth.get(S.selected) : null;
    if (t) {
      t.mesh.updateWorldMatrix(true, false);
      const c = t.centroid.clone().applyMatrix4(t.mesh.matrixWorld);
      const b = t.ax.b.clone().applyQuaternion(
        t.mesh.parent ? t.mesh.parent.quaternion : new THREE.Quaternion());
      m.position.copy(c).addScaledVector(b, 28);
    } else {
      m.position.set(0, 6, 46);
    }
  }

  function clearMedia() {
    if (!S.mediaMesh) return;
    S.scene.remove(S.mediaMesh);
    if (S.mediaMesh.material.map) S.mediaMesh.material.map.dispose();
    S.mediaMesh.material.dispose();
    S.mediaMesh.geometry.dispose();
  ...
  S.api = {
    ...
    setMediaImage: setMediaImage, clearMedia: clearMedia, runStepSeq: runStepSeq,
  ...
  function animate() {
    ...
    if (S.mediaMesh && S.mediaMesh.visible) S.mediaMesh.quaternion.copy(S.camera.quaternion);
