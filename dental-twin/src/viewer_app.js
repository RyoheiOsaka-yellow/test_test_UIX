...
(function () {
  ...
  function recolorAll() {
    ...
    const ga = CONFIG.GINGIVA_ALPHA[S.stage];
    S.gingiva.forEach(function (m) {
      m.material.opacity = ga;
      m.material.transparent = ga < 1;
      m.material.depthWrite = ga >= 1;
    });
    S.bone.forEach(function (m) { m.visible = CONFIG.BONE_VISIBLE[S.stage] || S.layers.bone; });
    updatePerioMarkers();
  ...
  function focusTooth(fdi) {
    ...
    S.desired.target.copy(t.centroid);
    S.desired.dist = 62;
    const c = t.centroid;
  ...
  function pick(cx, cy, isClick) {
    ...
    if (!hits.length) {
      ...
      if (isClick) { S.selected = null; S.needsRecolor = true; renderDetail(null); }
      return;
    }
    const h = hits[0];
  ...
  function surfaceAt(t, hit) {
    const p = hit.point.clone().sub(t.centroid);
  ...
  function bindUI() {
    ...
    open.addEventListener('input', function () { S.openDeg = parseFloat(open.value); });
  ...
  function animate() {
    ...
    S.cur.theta += (S.desired.theta - S.cur.theta) * L;
    ...
    S.camera.position.set(
      S.cur.target.x + S.cur.dist * sp * Math.sin(S.cur.theta),
      S.cur.target.y + S.cur.dist * cp,
      S.cur.target.z + S.cur.dist * sp * Math.cos(S.cur.theta)
    );
    S.camera.lookAt(S.cur.target);

    if (S.mandible) {
      const ex = S.mandible.userData || {};
      const hp = ex.hinge_point || [0, -12, -75];
      const a = -S.openDeg * Math.PI / 180;
