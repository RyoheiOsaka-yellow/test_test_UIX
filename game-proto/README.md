...
| ファイル | 内容 |
| --- | --- |
| `webgl/index.html` | **3D 版**。WebGL2 の自前 Gaussian スプラットシェーダで地図を描き、部屋をドールハウス表示する。家具をドラッグでき、ロボットの視点（現実／地図）を小窓で並べる |
| `interactive/index.html` | **2D 版**。上から見た部屋。仕組み（光線ごとの矛盾）が一番見やすい |
| `game-prototype.html` | 上の 2 つと報告を 1 ファイルにまとめた配布版（`build_standalone.py` で生成） |

3D 版は CPU 版と同じ構造で、センサ（96×72 の RGB-D）と GaME のロジックは JS、描画だけが WebGL。
Gaussian は等方（CPU 版が等方正則化を掛けているのに合わせた）。最適化は色・深度 L1 損失を勾配ではなく
直接の規則（観測面より手前なら不透明度を下げる、一致するなら上げる）に置き換えている。
シミュレーションは固定タイムステップ＋1 フレームあたりの実時間予算で回るので、描画が遅い環境では
スローモーションになるが結果は変わらない。
