...
/* ---------- AI動画/画像モデル別プロンプト方言 ---------- */
const PROMPT_MODELS = [
  { id: "orchestrate", label: "Seedance 2.5 (参照オーケストレーション)",
    hint: "公式型: REFERENCES→ACTION→CAMERA→PERFORMANCE→ENVIRONMENT→PRESERVE/CHANGE→PRIORITY。参照ごとに役割と「使わない情報」を指定する" },
...
/* =========================================================
 * 参照オーケストレーション (Seedance 2.5世代の作り方)
 *
 * 1回の生成で画像30枚・動画10本・音声10本を参照として受け取れる以上、
 * プロンプトが全部を説明する前提では作られていない。
 * 判断基準は1つ —「その情報を一番正確に持っているのは誰か」。
 * 動きの正解を持つのは動きを撮った動画であって、それを説明した文章ではない。
 * そこでプロンプトの仕事を「映像の描写」から「素材の役割分担の指揮」に変える。
 * ======================================================= */

/* 参照素材の役割。use = そこから使う情報 / avoid = そこから拾わせない情報。
 * 役割を与えると同時に「使わないもの」を明示しないと、参照と文章が
 * 別々のことを言い出して破綻する。 */
const REF_ROLES = [
  { id: "identity", en: "character identity reference", label: "同一性 (人物の顔)", media: "image", short: "同一性",
    use: "character identity, face, hair",
    avoid: "location, clothing, text, people, story, camera work",
    note: "顔の正解は画像が持つ。文章では同一性を保てない。" },
  { id: "product", en: "product design reference", label: "商品の形", media: "image", short: "商品",
    use: "product design, exact shape, scale, logo placement",
    avoid: "location, lighting mood, story, camera work",
    note: "商品の正確な形は画像でしか固定できない。" },
  { id: "scene", en: "scene and lighting reference", label: "シーン/世界観 (first frame)", media: "image", short: "シーン",
    use: "scene design, materials, lighting, color, atmosphere",
    avoid: "character identity, body motion, story",
    note: "公式例と同じ配分。素材・照明・色・雰囲気をここから取る。" },
  { id: "style", en: "style and color reference", label: "スタイル/色", media: "image", short: "スタイル",
    use: "color palette, contrast, grain, finish",
    avoid: "subject, composition, location, story",
    note: "仕上がりの質感だけを担当させる。" },
  { id: "motion", en: "motion reference", label: "動き (身体・立ち位置・視線)", media: "video", short: "動き",
    use: "body rhythm, arm gestures, walking rhythm, gaze, blocking, timing, body-linked camera motion",
    avoid: "location, clothing, text, people, story",
    note: "動きの正解は動画が持つ。部位ごとに分解した文章より参照1本が強い。" },
  { id: "whitemodel", en: "clay render for camera and blocking", label: "白模型 (空間・カメラ軌道)", media: "video", short: "白模型",
    use: "camera path, pacing, shot size changes, subject trajectory, floor plan",
    avoid: "texture, material, color, lighting, final look",
    note: "空間・カメラパス・相対位置が最終的な質感より重要なときに有効。" },
  { id: "greenscreen", en: "green screen performance plate", label: "グリーンスクリーン演技", media: "video", short: "GS演技",
    use: "performance, product demonstration, timing, gaze",
    avoid: "background, environment lighting, color grade",
    note: "人の動作・商品の実演を人間が撮って渡す。" },
  { id: "audio", en: "voice reference", label: "音声 (声・喋り)", media: "audio", short: "音声",
    use: "voice timbre, delivery, timing",
    avoid: "background noise, room tone",
    note: "声の正解は音声素材が持つ。" },
];

/* 残すもの / 変えるもの。この区別を書かないとAIは全部を作り直す自由を持つ */
const PRESERVE_ITEMS = [
  ...
  { id: "identity", label: "人物の同一性", en: "character identity" },
  { id: "productShape", label: "商品の形", en: "the product design" },
  { id: "shotOrder", label: "ショット順", en: "the shot order" },
  { id: "voice", label: "声", en: "the voice" },
];
const CHANGE_ITEMS = [
  ...
  { id: "product", label: "商品", en: "the product" },
];

/* 守らせる強さ3段階。全部を固定すると不自然になり、
 * 全部をおまかせにすると狙いから外れる。どこを緩めるかの設計が仕事になる */
const LOCK_LEVELS = [
  { id: "lock", label: "固定", en: "LOCKED — reproduce exactly, do not reinterpret" },
  { id: "guide", label: "誘導", en: "GUIDED — direction only, interpretation allowed" },
  { id: "free", label: "おまかせ", en: "FREE — your choice" },
];

/* 要素ごとの「正解を持たせる先」。source は根拠のある置き場所 */
const LOCK_TARGETS = [
  { id: "identity", label: "顔・人物の同一性", en: "character identity", source: "画像", def: "lock",
    why: "文章では同一性を保てない" },
  { id: "motion", label: "身体の動き・立ち位置・視線・タイミング", en: "body motion, blocking, gaze and timing", source: "動画", def: "lock",
    why: "公式が動画参照の制御対象として明記" },
  { id: "space", label: "空間の構造・カメラの軌道", en: "spatial layout and camera path", source: "白模型", def: "guide",
    why: "空間・カメラパス・相対位置が質感より重要なときに有効" },
  { id: "product", label: "商品の形・実演", en: "product design and demonstration", source: "画像/GS素材", def: "lock",
    why: "形の正解は画像、実演の正解は撮った素材が持つ" },
  { id: "mood", label: "世界観・場所・意味・優先順位", en: "world, place, meaning and priorities", source: "プロンプト", def: "guide",
    why: "文章が最も強い領域" },
  { id: "props", label: "家具の形・小物・背景の細部", en: "furniture, props and background details", source: "AIに任せる", def: "free",
    why: "固定する必要がない" },
];

/* 動きの質 — 「◯◯という名前の動き」ではなく、速さ・雑さ・誰に向けた動きかで書く。
 * 概念名は解釈の幅がそのまま出力の幅になるため使わない */
const MOTION_SPEEDS = [
...
const MOTION_TOWARDS = [
  ...
  { id: "person", label: "相手に向けて", en: "directed at the other person" },
];
/* 演技の温度 */
const PERF_TEMPS = [
  { id: "private", label: "一人でやっている", en: "private, unaware of being watched" },
  { id: "shown", label: "誰かに見せている", en: "aware of an audience, performing for someone" },
  { id: "observed", label: "観察されている (気づいていない)", en: "observational, the subject does not acknowledge the camera" },
];
